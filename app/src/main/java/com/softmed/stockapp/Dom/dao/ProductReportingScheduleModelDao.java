...
public interface ProductReportingScheduleModelDao {
    ...
    @Query("select *" +
            ...
            "INNER JOIN Product ON ProductReportingSchedule.product_id = Product.id " +
            "WHERE Product.id = :productId")
    LiveData<List<ProductReportingSchedule>> getMissedProductReportings(int productId);


    @Query("select * from ProductReportingSchedule WHERE product_id = :productId and status=0 ")
